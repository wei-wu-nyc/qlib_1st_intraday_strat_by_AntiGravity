...
def run_full_day_strategy():
    print("🚀 Starting Full Day Multi-Trade Strategy Analysis...")
    ...
    # --- Config ---
    periods = ['train', 'valid', 'test'] # Added train
    ...
    for period in periods:
        ...
        # --- Benchmark Data (SPY) ---
        spy_df = df.xs('SPY', level=1)['close']
    ...
    def merge_periods(p_list, name):
        ...
        reb_qqq_eq = [1.0] # New
        bench_eq = [1.0]
        for r in full_strat_ret:
            strat_eq.append(strat_eq[-1] * (1 + r))
        for r in full_subset_ret:
            subset_eq.append(subset_eq[-1] * (1 + r))
        for r in full_conc_ret:
            conc_eq.append(conc_eq[-1] * (1 + r))
        for r in full_reb_ret:
            reb_eq.append(reb_eq[-1] * (1 + r))
        for r in full_conc_qqq_ret:
            conc_qqq_eq.append(conc_qqq_eq[-1] * (1 + r)) # New
        for r in full_reb_qqq_ret:
            reb_qqq_eq.append(reb_qqq_eq[-1] * (1 + r)) # New
        for r in full_bench_ret:
        ...
        bench_stats = calc_stats(full_bench_ret)
        
        # Trade Stats (Targeting Concentrated Strategy for Display as requested)
        # Dynamic strategy has partial fills/netting which messes up per-trade arithmetic.
        # User requested using Max 2 (Concentrated) as the reference for trade stats.
        
        merged_conc_trades = []
        for p in p_list:
            r_conc = period_results[p]['strategy_conc']
            if not r_conc.trades.empty:
                merged_conc_trades.append(r_conc.trades)
        ...
        if not df_trades.empty:
            if 'return_pct' in df_trades.columns:
                ...
                # Update conc_stats with these trade metrics
        ...
        if not df_trades.empty:
            if 'return_pct' in df_trades.columns:
                ...
                for inst, g in grp:
                    # pnl column exists in df_trades
        ...
        return {
            ...
            'conc_stats': conc_stats, # metrics now in here
            'reb_stats': reb_stats,
    ...
    final_data['train'] = merge_periods(['train'], 'Train Period') # Kept for 'All'
...
<head>
    ...
    <style>
        ...
        .stats-container {{ display: grid; grid-template-columns: repeat(5, 1fr); gap: 15px; margin-bottom: 30px; }}
        .stats-box {{ background: rgba(255,255,255,0.05); padding: 15px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); }}
        .stats-title {{ color: #00d9ff; border-bottom: 1px solid #444; padding-bottom: 10px; margin-bottom: 15px; font-size: 16px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }}
        
        .stat-row {{ display: flex; justify-content: space-between; margin-bottom: 8px; font-size: 14px; }}
        .stat-val {{ font-weight: bold; }}
        
        .chart-container {{ width: 100%; height: 500px; background: rgba(0,0,0,0.2); border-radius: 8px; margin-bottom: 30px; }}
...
<body>
    ...
    <div class="stats-container">
        <!-- Strategy Stats (Full) -->
        <div class="stats-box">
            <div class="stats-title">Full Strategy (All ETFs)</div>
            <div id="strat-stats"></div>
        </div>
        
        <!-- Strategy Stats (Subset) -->
        <div class="stats-box">
            <div class="stats-title" style="color: #ff9900;">QQQ+IWM (Max 5)</div>
            <div id="subset-stats"></div>
        </div>
        ...
        <!-- Benchmark Stats -->
        <div class="stats-box">
            <div class="stats-title" style="color: #888;">Benchmark (SPY)</div>
    ...
    <h3>Instrument Analysis (Concentrated Strategy - Max 2)</h3>
    ...
    <script>
        ...
        function render(key) {{
            ...
            // Render Stats
            const sMetrics = ['Total Return', 'Annualized', 'Sharpe', 'Max DD', 'Vol', 'Avg Daily Ret', 'Total Days', 'Avg Invested'];
            ...
            renderStats('strat-stats', d.strategy_stats);
            renderStats('subset-stats', d.subset_stats);
            ...
            Plotly.newPlot('chart', [trace1, trace3, trace4, trace5, trace2], layout);
            
            // Render Table
            document.getElementById('inst-table').innerHTML = d.instrument_html;
        }}
    ...
    with open('results/active/full_day_strategy_dashboard.html', 'w') as f:
        f.write(html)
    print("✅ Dashboard updated: results/active/full_day_strategy_dashboard.html")
